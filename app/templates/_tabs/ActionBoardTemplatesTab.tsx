...
/**
 * Action Board Templates tab — admin CRUD for `milestone_templates`.
 *
 * [2026-08-05 per Andy] Moved here from /clients/templates, which is now
 * a back-compat redirect. It was the one Templates surface the 2026-06-03
 * consolidation missed: three sidebar entries became /templates with
 * Messages + Tasks + Deliverables tabs, but this one was a sub-route of
 * /clients reachable only via a button on the clients toolbar and a
 * "Manage templates…" item in the per-client Action Board dropdown. Two
 * places to look for the same kind of thing.
 *
 * List view, row actions (edit, duplicate, delete, set-as-default), and
 * an editor dialog for create/edit. Templates are GLOBAL (not per-client),
 * which is what makes a shared admin surface the right home rather than
 * the per-client Context popup's Action Board tab.
 *
 * The Context popup still has its "Templates" dropdown for applying /
 * saving-current-as. This tab is the management hub for everything else
 * (rename, modify, delete, mark default).
 *
...
import {
  Plus, Settings, Trash2, Copy, Star, StarOff, Pencil, ChevronUp, ChevronDown, Search, X, ListChecks, MoreHorizontal,
} from 'lucide-react';
import {
...
export default function ActionBoardTemplatesTab() {
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-4">
      {/* Toolbar row matches the Tasks + Deliverables tabs: one-line
          blurb on the left, primary CTA on the right. The page's old
          back-link + PageHeader are gone — the /templates shell owns
          the header now. */}
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <p className="text-sm text-ink-warm-500">
          Reusable milestone sets you can apply to any client&apos;s Action Board.
        </p>
        <Button variant="brand" onClick={openNew} disabled={loading}>
