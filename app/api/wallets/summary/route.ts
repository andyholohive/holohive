...
export async function GET() {
  ...
  // One read — pull all rows. We page if it ever exceeds 10k.
  ...
  // Per-event participation: how many wallets did each event reach?
  // Also chain-split per event so the Overview can show "DataHaven
  // Event 1: 500 wallets (480 EVM, 20 Solana)" style rows.
  ...
  // Sorted by reach descending — the natural Overview ordering.
  const events_by_reach = Array.from(eventStats.entries())
    .map(([name, s]) => ({ name, ...s }))
    .sort((a, b) => b.total - a.total);

  // Campaign overlap matrix: for each ordered pair (A, B), count
  // wallets that appear in BOTH. Symmetric — matrix[A][B] === matrix[B][A].
  // Diagonal === each event's total reach. The UI renders as a square
  // grid; we send a flat record-of-records keyed by event name.
  ...
  return NextResponse.json({
    ...
    retention,
    retention_pct: {
      // Pre-computed so the UI doesn't have to redo / handle division-by-zero.
    ...
    events_by_reach,
    overlap,
    // Convenience: cross-event retention rate = wallets in 2+ events
    // / total. Headline number for the KPI strip.
