...
// [2026-06-05] DialogDepthContext — tracks how many DialogContents are
// nested in the React tree. The outermost (depth 1) renders the dark
// `bg-black/80` overlay; nested ones (depth 2+) render a transparent
// overlay so the dark backdrops don't stack and double-darken the
// screen. Each overlay still mounts (Radix needs it for outside-click
// detection + the modal-stack focus trap); only the visual fill
// changes. Context propagates through DialogPortal because the React
// tree is preserved across portals even though the DOM is reparented.
const DialogDepthContext = React.createContext(0);

const DialogOverlay = React.forwardRef<
  React.ElementRef<typeof DialogPrimitive.Overlay>,
  React.ComponentPropsWithoutRef<typeof DialogPrimitive.Overlay> & {
    /** When true, render a fully transparent overlay so nested
     *  dialogs don't add a second dark layer. */
    nested?: boolean;
  }
