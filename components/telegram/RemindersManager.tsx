...
/**
 * `embedded` drops the page chrome so this can render inside the
 * Telegram page's Reminders tab. Reminders are Telegram routes — the
 * only difference from the ones on the Routes tab is that these fire on
 * a schedule rather than on an event — so they belong beside them
 * rather than behind their own sidebar entry.
 */
export function RemindersManager({ embedded = false }: { embedded?: boolean }) {
  ...
  // "Live" means the rule can actually send: switched on AND pointed at a
  // real chat. Counting is_active alone reported 8 active while only 3 of
  // them could fire — the same gap the badge had. A disabled rule doesn't
  // need a chat, so it isn't counted as unconfigured either.
  const liveCount = rules.filter(r => r.is_active && isRouted(r)).length;
  const unconfiguredCount = rules.filter(r => r.is_active && !isRouted(r)).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionHeader
        label="Rules"
        dot="brand"
        counter={`${rules.length} rule${rules.length === 1 ? '' : 's'} · ${liveCount} live${
          unconfiguredCount > 0
            ? ` · ${unconfiguredCount} not routed${showUnconfiguredOnly ? ' · filtered' : ''}`
            : ''
        }`}
        first
      />
        ...
        <div className="flex items-start gap-3 px-4 py-3 rounded-lg border border-amber-200 bg-amber-50">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-amber-900">
              {unconfiguredCount} switched-on rule{unconfiguredCount === 1 ? '' : 's'} {unconfiguredCount === 1 ? 'has' : 'have'} no chat to send to
            </p>
            <p className="text-xs text-amber-700 mt-0.5">
              The engine skips these entirely, so their last-run time never advances. Open the rule, pick a chat, and save.
            </p>
          </div>
          <Button
            size="sm"
      ...
      <div className="grid gap-4">
        {(() => {
          const visibleRules = showUnconfiguredOnly
            ? rules.filter(r => r.is_active && !isRouted(r))
            : rules;
          if (rules.length === 0) {
